...
if [[ -z "$1" ]]; then  # No arguments
  for proj in `ls -d -1 proj-*`; do
    echo "argocd app create -f ${proj}/env/${DEPLOY_ENV}/application.yaml --grpc-web"
  done
...
if [[ -z "$1" ]]; then  # No arguments
  for proj in `ls -d -1 proj-*`; do
    echo "argocd --grpc-web app create -f ${proj}/env/${DEPLOY_ENV}/application.yaml --grpc-web"
    argocd --grpc-web app create -f "${proj}/env/${DEPLOY_ENV}/application.yaml" --grpc-web
  done
else # Argument received
  proj="$1"
  echo "argocd --grpc-web app create -f ${proj}/env/${DEPLOY_ENV}/application.yaml --grpc-web"
